...
class GeneSolve:
    ...
    def __init__(self, pop_size, epoch, cross_prob, mutate_prob, alpha, print_batch=10):
        ...
        self.best = None
        self.genes = np.array(
                [''.join([random.choice(range(min_netweight, max_netweight)) for i in range(self.width)]) for j in range(self.pop_size)])
    ...
    def mutate(self):
        ...
        for i in ready_index:
            if np.random.uniform(0, 1) <= self.mutate_prob:
                ...
                t0[loc] = str(random.choice(range(min_netweight, max_netweight)))
                self.genes[i] = ''.join(t0)

    def get_adjust(self):
        """计算适应度"""
        x = random.choice(range(self.pop_size))
        return x
